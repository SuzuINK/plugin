...
public class MenuManager {
    ...
    public void openDungeonsMenu(Player player) {
        ...
        List<Dungeon> availableDungeons = plugin.getDungeonManager().getAvailableDungeons(playerData.getLevel());

        Inventory menu = Bukkit.createInventory(null, 54, ChatColor.translateAlternateColorCodes('&', "&cDonjons Disponibles"));
        
        int slot = 10;
        for (Dungeon dungeon : availableDungeons) {
            if (slot % 9 == 8) slot += 3;
            
            ItemStack dungeonButton = createItem(Material.NETHER_BRICK, "&6" + dungeon.getName(), Arrays.asList(
                "&7" + dungeon.getDescription(),
                "",
                "&fNiveau requis: &e" + dungeon.getMinLevel(),
                "&fJoueurs max: &e" + dungeon.getMaxPlayers(),
                "&fTemps limite: &e" + (dungeon.getTimeLimit() / 60) + " minutes",
                "",
                "&eCliquez pour rejoindre!"
            ));
            menu.setItem(slot, dungeonButton);
            slot++;
        }
    ...
    public void openQuestsMenu(Player player) {
        Inventory menu = Bukkit.createInventory(null, 54, ChatColor.translateAlternateColorCodes('&', "&eQuêtes"));
        
        // TODO: Implémenter l'affichage des quêtes
